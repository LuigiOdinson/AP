...
class CompLamp:
    def __init__(self, parent, width, order, color="red", *args, **kwargs):
        self.frame = tkk.Frame(parent.frame, *args, **kwargs)
        self.canvas = tk.Canvas(self.frame, width=width, height=width,
                                bg="gray", highlightthickness=0)
        self.canvas.pack()
        self.color = color
        offset = width//8
        self.lamp = self.canvas.create_oval(offset, offset, 7*offset, 7*offset,
                                            fill="black")
...
class CompTrafficLight:
    def __init__(self, root, wd, initial_color="red", *args, **kwargs):
        ...
        self.frame = tkk.Frame(root, width=wd, *args, **kwargs)
